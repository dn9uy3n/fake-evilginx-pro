...
| 18 | **Google real-browser relay (an toàn botguard)** | 🟡 đã deploy | `tools/relay/bgrelay.py` + route fork `/__relay/*`: victim đăng nhập trên trang sao của ta, sidecar patchright (chromium CDP harden, headful Xvfb) đăng nhập THẬT trên accounts.google.com qua exit residential — botguard luôn thấy browser thật trên origin thật. Capture email/password + trọn bộ cookie `.google.com` (`~/bgrelay-store/`). Vượt origin-bound botguard chặn mọi phishlet MITM cổ điển |
