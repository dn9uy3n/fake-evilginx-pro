...
var botguard_ja4 = flag.String("bg-ja4", "", "Botguard JA4 allowlist prefixes, comma separated (e.g. t13d1516,t13d90)")
var botguard_det_cidrs = flag.String("bg-det-cidrs", "", "Detonator client IP CIDR ranges (comma separated)")
var botguard_trusted = flag.String("bg-trusted", "", "Trusted reverse-proxy CIDRs — botguard skips scoring for these clients (e.g. 127.0.0.1/32 when behind InfraGuard)")
...
var no_watch = flag.Bool("no-watch", false, "Disable phishlet directory auto-reload")
...
func main() {
	...
	cfg.CleanUp()
	cfg.PhishletsDir = phishlets_path
	if !*no_watch {
		go core.WatchPhishlets(cfg, phishlets_path)
	}
	...
	hp.SetBotguardJA4(*botguard_ja4)
	hp.SetBotguardDetCIDRs(*botguard_det_cidrs)
	hp.SetBotguardTrustedCIDRs(*botguard_trusted)
